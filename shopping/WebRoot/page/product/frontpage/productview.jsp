...
<link href="/shopping/css/product/product.css" rel="stylesheet" type="text/css">
<link href="/shopping/css/global/topcommend.css" rel="stylesheet" type="text/css">
...
<div id="ContentBody"><!-- 页面主体 -->
    <c:set var="out" value="&gt;&gt; <em>${product.name}</em>"/><c:forEach items="${stypes}" var="type" varStatus="statu">
		<c:set var="out" value=" &gt;&gt; <a href='/shopping/control/frontProductAction_findProduct?typeid=${type.typeid}'>${type.name}</a> ${out}"/>
	</c:forEach>
	 	 <div id="position"> 您现在的位置：<a href="/shopping/control/frontProductAction_findProduct" name="linkHome">巴巴运动网</a> <span id="uc_cat_spnPath"><c:out value="${out}" escapeXml="false"></c:out></span></div>
 <div class="browse_left"><!-- 页面主体 左边 --> 
	...
	<DIV id="topcommend" align="left">
	       <DIV id="newtop"><IMG height=13 src="/shopping/images/global/sy2.gif" width=192></DIV>
	       <DIV id="newlist">
		  <DIV id="newmore">
...
<form action="<s:url action="cartAction_addItem" namespace="/control"/>" method="post" name="cart">
...
<c:forEach items="${product.styles}" var="style"><c:if test="${style.visible}"><c:set var="currentimage" value="${style}"/><c:set var="imagecount" value="${imagecount+1}"/></c:if></c:forEach>
		<div class="right_left">
				<div id="main_image" onclick="JavaScript:bigImageBrowse()" style="cursor:hand;background:url(${currentimage.logopath}) center center no-repeat"><img src="/shopping/images/global/product_blank.gif" WIDTH="200" HEIGHT="240"/></div>
					<img src="/shopping/images/global/zoom+.gif" onclick="JavaScript:bigImageBrowse()" style="cursor:hand;"/>
				</div>
				<div class="right_right">									
						<div class="right_title"><b>${product.name}</b></div>
						<div class="right_desc">
							<ul><li class="li2">商品编号：A${product.productid}<font color="#CC0000">（电话订购专用）</font></li>
				...
				<div class="right_desc">
						<c:if test="${imagecount==1}">
							<INPUT TYPE="hidden" NAME="styleid" value="${currentimage.styleid }">
								...
								<INPUT TYPE="hidden" id="productImage_${currentimage.styleid }" value="${currentimage.name}">
								<INPUT TYPE="hidden" id="productPrototypeImage_${currentimage.styleid }" value="${currentimage.logopath}">
						</c:if>
								<c:if test="${imagecount>1}">
												<img src="/shopping/images/global/init.gif" width="0" height="0">
								...
								<c:forEach items="${product.styles}" var="style"><c:if test="${style.visible}">
									<option value="${style.styleid }" <c:if test="${style.styleid==currentimage.styleid}">selected </c:if>>${style.name }</option></c:if></c:forEach></SELECT></li>
								<c:forEach items="${product.styles}" var="style"><c:if test="${style.visible}">
											    <INPUT TYPE="hidden" id="productPrototypeImage_${style.styleid }" value="${style.logopath}"></c:if></c:forEach>
											</c:if>
									...
									<ul>
										<li>市场价：<s>${product.marketprice}</s> 元 <font color='#ff6f02'>本站价：<b>${product.sellprice} 元</b></font> 节省：<font color='#ff6f02'>${product.savedPrice }</font>元</li>
									  	<li class="right_img"><INPUT TYPE="image" SRC="/shopping/images/global/sale.gif"></li><!-- 购买按钮 --><li class="guopiprice">
										[ <IMG src="/shopping/images/global/2j4.gif" border="0">&nbsp;<A href="http://www.babasport.com/cache/news/6/9.shtml" target="_blank">配送说明</A> ]&nbsp;&nbsp;&nbsp;&nbsp;[ <IMG src="/shopping/images/global/2j4.gif" border="0">&nbsp;<A href="http://www.babasport.com/cache/news/4/24.shtml" target="_blank">付款方式</A> ]
										</li>
									</ul>									
							</div>
</div>
