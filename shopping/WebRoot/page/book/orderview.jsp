...
function ActionEvent(methodname, orderid){
	...
	window.location.href = '<s:url action="dealOrderAction"/>&method='+ methodname;
}
  ...
  <tr>
    <td><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
      ...
      <table width="100%" border="0" align="center" cellpadding="3" cellspacing="2">
        <tr>
          <td colspan="4" bgcolor="#FFFFFF"><strong>订购者信息 </strong> <a href="<s:url action="/control/order/manage"/>?method=modifyContactInfoUI&contactid=${order.orderContactInfo.contactid }">修改</a></td>
          <td align="center" bgcolor="#FFFFFF">支付方式</td>
          <td colspan="2" bgcolor="#FFFFFF">${order.paymentWay.name }  <a href="<s:url action="/control/order/manage"/>?method=modifyPaymentWayUI&orderid=${order.orderid }">修改</a></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4" bgcolor="#FFFFFF"><strong>收货人信息</strong> <a href="<s:url action="/control/order/manage"/>?method=modifyDeliverInfoUI&deliverid=${order.orderDeliverInfo.deliverid }">修改</a></td>
          <td align="center" bgcolor="#FFFFFF">送货方式</td>
          <td colspan="2" bgcolor="#FFFFFF">${order.orderDeliverInfo.deliverWay.name }  <a href="<s:url action="/control/order/manage"/>?method=modifyDeliverWayUI&orderid=${order.orderid }">修改</a></td>
        </tr>
        <tr>
...
<c:forEach items="${order.items}" var="item">
        <tr>
          ...
          <td align="center" bgcolor="#FFFFFF">${item.amount } <a href="<s:url action="/control/order/manage"/>?method=modifyProductAmountUI&orderitemid=${item.itemid }">修改</a></td>
          <td align="center" bgcolor="#FFFFFF"><a href="JavaScript:deleteOrderItem('${item.itemid }','${order.orderid }')">删除</a></td>
        </tr>
</c:forEach>
        <tr>
          <td colspan="7" align="right" bgcolor="#FFFFFF"><p>商品合计：￥${order.productTotalPrice }元&nbsp;&nbsp;配送费：￥${order.deliverFee }元 <a href="<s:url action="/control/order/manage"/>?method=modifyDeliverFeeUI&orderid=${order.orderid}">修改</a>&nbsp;&nbsp;订单合计：￥${order.totalPrice }元<br />
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="2">
  <tr>
    ...
    <input type="button" value="打印订单" onclick="JavaScript:winOpen('<s:url action="/control/order/manage"/>?method=printOrder&orderid=${order.orderid }','打印',700,450)"/>&nbsp;
	<input type="button" value="解锁退出" onclick="JavaScript:window.location.href='<s:url action="/control/order/manage"/>?method=employeeUnlockOrder&orderid=${order.orderid }'"/>
	</td>
  </tr>
</table>
<br />
<table width="90%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr>
    <td colspan="2"  bgcolor="6f8ac4"><FONT COLOR="#FFFFFF">客服留言</FONT> &nbsp; <input type="button" value="客服留言" onclick="JavaScript:window.location.href='<s:url action="orderManageAction_addMessageUI" namespace="control"/>'"/></td>
  </tr>
  <tr>
